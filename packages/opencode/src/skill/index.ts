...
// Built-in skill that ships with mycode. The model's intuition for what an
// mycode.json should look like is often wrong, and mycode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch mycode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_OPENCODE_SKILL_NAME = "customize-opencode"
const CUSTOMIZE_OPENCODE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating mycode's own configuration: mycode.json, mycode.jsonc, files under .mycode/, or files under ~/.config/mycode/. Also use when creating or fixing mycode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring mycode itself."
